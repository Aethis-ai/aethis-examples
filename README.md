...
Engine accuracy: 100% across 225 scenarios spanning four rule domains, where frontier LLMs score 63–100% (Simpson 2026 §3). The construction-CAR scenarios below are one domain from that dataset.
...
```bash
export AETHIS_API_KEY=ak_...    # free at https://api.aethis.ai
uv run run_tests.py construction-all-risks/
```
...
curl -X POST https://api.aethis.ai/api/v1/public/decide \
  ...
  -d '{"ruleset_id": "<ruleset-id>", "field_values": {"space.crew.species": "Vogon"}}'
```
