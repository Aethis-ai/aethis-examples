...
def discover_ruleset(api_url: str, project_name: str, api_key: str | None = None) -> str:
    ...
    for r in rulesets:
        ...
        if normalized in section or section in normalized:
            return r["ruleset_id"]
    console.print(f"[red]ERROR:[/red] No ruleset found matching [bold]{project_name}[/bold]")
    sys.exit(1)


def get_schema(api_url: str, ruleset_id: str, api_key: str | None = None) -> dict:
    resp = requests.get(f"{api_url}/api/v1/public/rulesets/{ruleset_id}/schema", headers=_auth_headers(api_key), timeout=15)
    resp.raise_for_status()
    data = resp.json()
    return {f["field_id"]: f for f in data.get("fields", [])}


def decide(api_url: str, ruleset_id: str, field_values: dict, api_key: str | None = None) -> dict:
    payload = {
        "ruleset_id": ruleset_id,
        "field_values": field_values,
        "include_trace": True,
    }
    resp = requests.post(f"{api_url}/api/v1/public/decide", json=payload, headers=_auth_headers(api_key), timeout=30)
    resp.raise_for_status()
    return resp.json()
...
def main():
    ...
    ruleset_id = args.ruleset_id or discover_ruleset(args.url, project_name, api_key=args.api_key)

    # Load field schema for type-aware prompts
    schema = get_schema(args.url, ruleset_id, api_key=args.api_key)
    ...
    field_values: dict = {}
    result = decide(args.url, ruleset_id, field_values, api_key=args.api_key)
