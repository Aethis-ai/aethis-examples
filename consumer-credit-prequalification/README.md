...
Creates the rulebook, authors the `prequalification` ruleset via the
project pipeline, publishes it into the rulebook (Phase A.9 bridge),
promotes it to live, and smoke-tests with an adverse-credit applicant
(expected: immediate decline via S.3 early termination).

### Step by step

```bash
cd consumer-credit-prequalification
aethis rulebooks create "Consumer Credit Pre-Qualification" \
    --domain consumer_credit --slug aethis/consumer-credit-prequalification
aethis generate --poll
aethis test
aethis publish --rulebook aethis/consumer-credit-prequalification --ruleset-name prequalification
# Capture the printed ruleset_id, then:
aethis rulesets promote-to-live aethis/consumer-credit-prequalification prequalification <ruleset_id>
aethis rulebooks decide aethis/consumer-credit-prequalification \
    -i '{"credit.has_adverse_history": true}'
```
