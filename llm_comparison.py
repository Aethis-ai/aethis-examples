...
DEFAULT_MODELS = ["gpt-5.4", "gpt-5-mini", "claude-opus-4-7", "claude-sonnet-4-6"]
...
def discover_ruleset(api_url: str, project_name: str, api_key: Optional[str] = None) -> str:
    headers: dict = {}
    if api_key:
        headers["X-API-Key"] = api_key
    resp = requests.get(f"{api_url}/api/v1/public/rulesets", headers=headers, timeout=15)
...
def ask_engine(api_url: str, ruleset_id: str, test: dict, api_key: Optional[str] = None) -> str:
    headers: dict = {}
    if api_key:
        headers["X-API-Key"] = api_key
    resp = requests.post(
        f"{api_url}/api/v1/public/decide",
        json={"ruleset_id": ruleset_id, "field_values": test["inputs"]},
        headers=headers,
        timeout=30,
    )
...
def main():
    ...
    ruleset_id = args.ruleset_id or discover_ruleset(args.url, config["project"], args.api_key)
    n_runs = args.runs
    ...
    if ruleset_id:
        console.print(f"  [dim]Ruleset: {ruleset_id}[/dim]")
    console.print()
    ...
    for test in tests:
        ...
        engine_answer = ask_engine(args.url, ruleset_id, test, args.api_key) if ruleset_id else "no ruleset"
