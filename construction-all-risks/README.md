...
14 test cases covering the full exception chain and the v3.8 depth-extension scenarios:
...
### End-to-end (canonical script)

```bash
./scripts/publish-rulebook.sh
```

Runs the full converged 2-term flow: create the rulebook → author the
ruleset via the project pipeline (`aethis generate` + `aethis test`) →
publish into the rulebook with `aethis publish --rulebook --ruleset-name` →
`aethis rulesets promote-to-live` → `aethis rulebooks decide` smoke.

### Step by step

```bash
cd construction-all-risks
aethis rulebooks create "CAR — Defect Exclusion Endorsement" \
    --domain construction_insurance --slug aethis/construction-all-risks
aethis generate --poll
aethis test
aethis publish --rulebook aethis/construction-all-risks --ruleset-name policy_endorsement
# Capture the printed ruleset_id, then:
aethis rulesets promote-to-live aethis/construction-all-risks policy_endorsement <ruleset_id>
aethis rulebooks decide aethis/construction-all-risks \
    -i '{"car.claim.is_rectification": true}'
```
...
| `car.defect.was_known_prior` | boolean | Defect was known (or reasonably knowable) before commencement |
| `car.defect.has_engineer_assessment` | boolean | Independent chartered engineer assessment obtained |
| `car.component.installed_by_subcontractor` | boolean | Defective component installed/fabricated by a subcontractor |
